...
- Voice: the auto-read toggle plus a single `Voice Engine` link row showing the current engine; the engine picker and all engine-specific configuration (Volcengine, Voicebox) live on a dedicated Voice subpage
- Memory Profile: one link row to the memory detail page (showing the evidence count as its value) plus the MBTI picker — inferred signal rows are shown only on the detail page, never duplicated in Settings
