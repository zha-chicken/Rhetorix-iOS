...
推进规则是透明的：在任意一场有裁判评分的辩论中，某项技能的教练评分达到 4 分及以上即视为掌握，今日技能永远是路径上第一个未掌握的技能。路径是开放的，不设锁定，用户可以在技能路径卡片上点击任意步骤直接练习；首页和引导练习页都会显示当前所处的步骤（第 X / 5 步）。五项技能全部掌握后，今日技能会转为针对本地数据中最明显的弱点，数据不足时按完成场次轮换。
